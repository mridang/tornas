//! The observable instruments: current state read from the engine at collection
//! time. Registered once by [`observe`], after the engine has started. Each
//! callback reads a short-TTL [`Snapshot`] of the engine so one scrape does the
//! work once, and the returned handles are retained for the life of the process
//! (a dropped observable deregisters its callback).
//!
